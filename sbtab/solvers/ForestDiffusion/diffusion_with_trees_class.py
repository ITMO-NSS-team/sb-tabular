...
class ForestDiffusionModel():
  def __init__(self, 
    ...
    if self.p_in_one:
      if self.n_jobs == 1:
        ...
        pbar = tqdm(range(n_steps), desc="Training ForestDiffusion", unit="step")
        for i in pbar:
        ...
        print("Training ForestDiffusion models in parallel...")
        if self.n_batch > 0:
      ...
      if self.n_jobs == 1:
        ...
        pbar = tqdm(range(n_steps), desc="Training ForestDiffusion (per-feature)", unit="step")
        for i in pbar:
          for j in range(len(self.y_uniques)):
  ...
  def predict_over_c(self, X, i, j, k, dmat, expand=False, X_covs=None):
    ...
    use_dmat = dmat or (self.gpu_hist and self.model == 'xgboost')
    if use_dmat:
  ...
  def generate(self, batch_size=None, n_t=None, X_covs=None, max_batch_size=2048):
    total_size = self.b if batch_size is None else batch_size

    if X_covs is not None:
      assert X_covs.shape[0] == total_size
    ...
    for start_idx in range(0, total_size, max_batch_size):
      ...
      y0 = np.random.normal(size=(curr_size, self.c))
